...
def main(args=None):
    ...
    parameters['k_city'] = carla_bridge.get_param('k_city', 'k_city')
    ...
    try:
        ...
        if "k_city" in parameters and not parameters['passive']:
            if parameters["k_city"].endswith(".xodr"):
                carla_bridge.loginfo(
                    "Loading opendrive world from file '{}'".format(parameters["k_city"]))
                with open(parameters["k_city"]) as od_file:
                    data = od_file.read()
                carla_world = carla_client.generate_opendrive_world(str(data))
            else:
                if carla_world.get_map().name != parameters["k_city"]:
                    carla_bridge.loginfo("Loading k_city '{}' (previous: '{}').".format(
                        parameters["k_city"], carla_world.get_map().name))
                    carla_world = carla_client.load_world(parameters["k_city"])
            carla_world.tick()
