...
def acados_solver():
    ...
    W_acc = 0.2  # 가속도 입력 크기 가중치 (대폭 감소 - 빠른 가속 적극 허용)
    W_steer = 0.5  # 조향각 입력 크기 가중치 0.2
    W_steer_rate = 1.5  # 조향각 변화율 가중치
    W_v = 0.2  # 속도 error 가중치 (대폭 감소 - 속도 추종 우선순위 하락)
    W_lag = 1.0  # lag error 가중치 1.0
    W_con = 0.1  # contour error 가중치 1.0
    W_yaw = 0.4  # heading error 가중치 (terminal cost) 0.5
    m_term = -0.7 # terminal cost에서 stage cost 비율 감소시키는 가중치
