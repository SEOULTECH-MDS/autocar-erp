...
class PathTracker(Node):
    def __init__(self):
        ...
        # 시각화를 위한 퍼블리셔 추가
        self.curvature_pub = self.create_publisher(MarkerArray, '/curvature', 10)
        self.reference_point_pub = self.create_publisher(Marker, '/reference_point', 10)
    ...
    def mpc_control(self):
        ...
        if ov is not None and od is not None:
            ...
            for i in range(self.prediction_horizon):
                # 곡률 제약
                curvature_constraint = self.curvatures[i] <= self.max_curvature
                
                # 속도 제약 (곡률 기반)
                max_velocity = np.sqrt(self.max_lateral_accel / self.curvatures[i])
                velocity_constraint = self.velocity <= max_velocity
