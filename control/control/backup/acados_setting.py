...
from kinematic_bicycle_model import export_bicycle_model
import numpy as np
...
def acados_solver():
    ...
    MIN_ACCEL = -0.614  # minimum accel [m/ss] defualt: -1.0
    ...
    ocp.constraints.idxbx = np.array([0, 1, 2, 3])  # 상태 변수 인덱스
    ...
    R = np.diag([0.01, 0.01])  # 제어 입력 가중치 (delta, a)
